...
class PatchGCN_Surv(torch.nn.Module):
    ...
    def forward(self,  **kwargs):
        ...
        batch = data.batch # 不是batch size [instance num]
        edge_attr = None

        x = self.fc(data.x)  # [instance num,128]
        x_ = x 
        
        x = self.layers[0].conv(x_, edge_index, edge_attr)
        x_ = torch.cat([x_, x], axis=1)  # [instance num,256]
        for layer in self.layers[1:]:
            x = layer(x, edge_index, edge_attr)
            x_ = torch.cat([x_, x], axis=1)
        
        h_path = x_ #[instance num,512]
        h_path = self.path_phi(h_path)#[instance num,512]

        tmp = global_mean_pool(h_path, batch.long())  # [batch_size, hidden_channels]

        A_path, h_path = self.path_attention_head(h_path)  #[instance num,1],[instance num,512]
        A_path = torch.transpose(A_path, 1, 0)#[1,instance num]
        h_path = torch.mm(F.softmax(A_path, dim=1), h_path) #[1,512]
        h = self.path_rho(h_path).squeeze() # [512] 将tensor中大小为1的维度删除
        logits  = self.classifier(h).unsqueeze(0) # logits needs to be a [1 x 4] vector [1,4]
        Y_hat = torch.topk(logits, 1, dim = 1)[1]
        hazards = torch.sigmoid(logits)
        S = torch.cumprod(1 - hazards, dim=1) #横着累积
