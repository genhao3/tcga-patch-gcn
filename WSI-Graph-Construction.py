...
### CLAM Path
# clam_path = '/data_local2/ljjdata/TCGA/BRCA/'
...
def createDir_h5toPyG(h5_path, save_path):
    ...
    for h5_fname in pbar:
        ...
        try:
            wsi_h5_path = os.path.join(h5_path, h5_fname)
            save_pt_path = os.path.join(save_path, h5_fname[:-3]+'.pt')
            # if os.path.exists(save_pt_path):
            #     continue
            wsi_h5 = h5py.File(wsi_h5_path, "r")
            G = pt2graph(wsi_h5)
            torch.save(G, save_pt_path)
            wsi_h5.close()
        except OSError:
...
h5_path = '/data_local2/ljjdata/TCGA/LUSC/feat_dir/h5_files/'
save_path = '/data_local2/ljjdata/TCGA/LUSC/graph_files/'
